...
function openatrium_profile_details() {
  return array(
    'name' => 'Open Atrium - UVic Installation',
...
function _openatrium_atrium_modules() {
  return array(
    ...
    // Atrium contrib features
    'atrium_folders', 'atrium_uvic_custom',
    // Dashboard
...
function _openatrium_intranet_configure_check() {
  ...
  db_query("UPDATE {system} SET status = 0 WHERE type = 'theme' and name ='%s'", 'ginkgo_uvic');  
  variable_set('theme_default', 'ginkgo_uvic');
