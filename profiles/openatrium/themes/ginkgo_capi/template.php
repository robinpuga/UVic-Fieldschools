 ...
 */
function ginkgo_capi_icon_links(&$links) {
  if (!empty($links)) {
    foreach ($links as $k => $v) {
      ...
      $v['attributes']['class'] .= ' icon-'. _ginkgo_capi_icon_class($v['href']);
 ...
 */
function ginkgo_capi_preprocess_block(&$vars) {
 ...
 */
function ginkgo_capi_preprocess_context_block_editable_region(&$vars) {
 ...
 */
function ginkgo_capi_preprocess_help(&$vars) {
  $vars['layout'] = FALSE;
  $vars['links'] = '';
}
 ...
 */
function ginkgo_capi_preprocess_comment(&$vars) {
  // Add a time decay class.
  $decay = _ginkgo_capi_get_comment_decay($vars['node']->nid, $vars['comment']->timestamp);
 ...
 */
function ginkgo_capi_preprocess_node_form(&$vars) {
 ...
 */
function ginkgo_capi_designkit_image($name, $filepath) {
  if ($name === 'logo') {
 ...
 */
function ginkgo_capi_more_link($url, $title) {
 ...
 */
function ginkgo_capi_breadcrumb($breadcrumb) {
 ...
 */
function ginkgo_capi_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
 ...
 */
function ginkgo_capi_views_mini_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
  $tags[1] = t('Prev');
  $tags[3] = t('Next');
 ...
 */
function ginkgo_capi_node_preview($node = NULL, $show = FALSE) {
  static $output;
 ...
 */
function ginkgo_capi_content_multiple_values($element) {
 ...
 */
function ginkgo_capi_node_submitted($node) {
 ...
 */
function ginkgo_capi_comment_submitted($comment) {
  $comment->created = $comment->timestamp;
  return ginkgo_capi_node_submitted($comment);
}
 ...
 */
function ginkgo_capi_preprocess_views_view_fields(&$vars) {
  foreach ($vars['fields'] as $field) {
    if ($class = _ginkgo_capi_get_views_field_class($field->handler)) {
      $field->class = $class;
    }
  }
 ...
 */
function ginkgo_capi_preprocess_views_view_table(&$vars) {
  $view = $vars['view'];
  foreach ($view->field as $field => $handler) {
    if (isset($vars['fields'][$field]) && $class = _ginkgo_capi_get_views_field_class($handler)) {
 ...
 */
function _ginkgo_capi_get_views_field_class($handler) {
 ...
 */
function _ginkgo_capi_get_comment_decay($nid, $timestamp) {
  static $timerange;
 ...
 */
function _ginkgo_capi_icon_class($path) {
